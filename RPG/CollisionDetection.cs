...
namespace RPG
{
    class CollisionDetection : Actor
    {
            public CollisionDetection(float x, float y, char icon = ' ', ConsoleColor color = ConsoleColor.White) : base(x, y, icon, color)
            {
                _collisionRadius = 1;
            }

            public CollisionDetection(float x, float y, Color rayColor, char icon = ' ', ConsoleColor color = ConsoleColor.White) : base(x, y, rayColor, icon, color)
            {
                _collisionRadius = 1;
            }
            ...
            {
                Raylib.DrawText("You Win!!\nPress Esc to quit", 10, 10, 10, Color.BLUE);
            }
        }
    }
